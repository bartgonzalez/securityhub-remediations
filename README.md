...
The automated remediations in this workshop will be deployed by executing Cloud Custodian suppling the input of a policy file written in a simple YAML based DSL which defines rules, resulting in the generating and deploying a Cloudwatch Event rule configured to invoke a Lambda Function also generated and deployed by Cloud Custodian. The cloudformation in this workshop will deploy a AWS Cloud 9 Environment with an IDE to view source file and a terminal window to execute commands. A single EC2 instance will be started and used as a Test Target to demonstrate automated remediation of common issues. Security Hub and Guard Duty will be enabled.  IAM Roles, Policies, and an IAM user will be created.
...
3. You must run this workshop in a region support by [AWS Cloud9](https://docs.aws.amazon.com/general/latest/gr/rande.html#cloud9_region)
...
```
6. This step is to create the cloudformation stack for the workshop.
6.1. Open the AWS Console, navigate to Cloudformation, click on the "Create stack" button, select the "With new resources (standard)" option, then in the "Specify template" section, click "Upload a template file", then click "Choose file", and when the File Dialog window pops up, select the file downloaded in the prior step.
6.2. Click Next.
6.3. Enter in a Stack name, for example "SecurityHubRemediations".
6.4. Change the value of "EventEngine" to False.
6.5. Change the value of VpcId to "CreateNew", unless you have a good reason to want to use an existing VPC and if so, enter the VpcId instead
6.6. Click the "Next" button.
6.7. On the "Configure stack options" page, Click the "Next" button.
6.8. On the "Review" page, click the checkbox to the left of "I acknowledge that AWS Cloudformation might create IAM resources with custom names", as it will.
6.9. Click "Create Stack"
6.10, Wait until the stack if finished creating before starting the next section, as the Cloud9 environment needs to be enabled before you can follow the next steps.
